...
class OnboardingWizard(FramelessWindow):
    ...
    def _step_mic(self) -> QWidget:
        ...
        lay.addWidget(title, 0, Qt.AlignmentFlag.AlignHCenter)
        lay.addSpacing(7)
        sub = QLabel("Fale algo e veja se estamos te ouvindo bem.")
    ...
    def _step_shortcut(self) -> QWidget:
        ...
        lay.addWidget(title, 0, Qt.AlignmentFlag.AlignHCenter)
        lay.addSpacing(7)
        sub = QLabel("Segure pra falar, solte pra colar.")
        sub.setFont(theme.qfont(13.5))
        sub.setStyleSheet(f"color: {pal.text_secondary};")
        lay.addWidget(sub, 0, Qt.AlignmentFlag.AlignHCenter)
        ...
        dot = QWidget(); dot.setFixedSize(5, 5)
        dot.setStyleSheet(
            f"background: {theme.palette().state(theme.WARNING)}; border-radius: 2px;")
        warn.addWidget(dot, 0, Qt.AlignmentFlag.AlignVCenter)
        wl = QLabel("aperte uma 3ª tecla durante o hold pra cancelar")
        wl.setFont(theme.qfont(12.5))
        wl.setStyleSheet(f"color: {pal.text_tertiary};")
        warn.addWidget(wl)
        ww = QWidget(); ww.setLayout(warn)
        lay.addWidget(ww, 0, Qt.AlignmentFlag.AlignHCenter)
